ranksum.test=function(form,           # formula or name of variable in data
                 data,           # data.frame
                 paired=FALSE,  # if compare two paired samples, proceed with Wilcoxon Signed-Rank test;
                 # if compare two unpaired samples, proceed with Wilcoxon Rank-Sum Test
  ...
  
{
  ...
  wilcox.res=wilcox.test(form,data,
                          paired=paired,
  ...
  test.name=ifelse(paired==F,'Wilcoxon rank sum test/Mann-Whitney test','Wilcoxon Signed-Rank test')
  res.txt1=paste0("The median of ",y.name," was ",
                 txt.list(paste0(rpt.num(res.tbl["median",],dgt)," among the ",
                                 res.tbl["available",]," ",
                                 colnames(res.tbl),
                                 " observations")),
                 " with available data (p = ",rpt.num(wilcox.res$p.value,dgt),paste0(";",test.name,').'))
  
  grp=colnames(res.tbl)
  if (wilcox.res$p.value<=alpha){
    
    res.txt2=paste0("As the p-value is less than or equal to the significance level=", alpha,
                     ". We can conclude that the median of ",y.name[1]," for ", grp[1]," is significantly different from the median of ", y.name, " for ", grp[2],". ")
    
  } else {
    
    res.txt2=paste0("As the p-value is greater to the significance level=", alpha,
                     ". We can conclude that the median of ",y.name[1]," for ", grp[1]," is not significantly different from the median of ", y.name, " for ", grp[2],". ")
  ...
  if ((hdr>0)&&(rpt)) 
  {
    res.hdr=paste0("<h",hdr,">Compare median of ",y.name," by ",grp.name,"</h",hdr,">")
  ...
  if (fig>0) res.txt=paste(res.txt,paste0("Figure ",fig.num," provides box plot of ",y.name,
                                      " by ",grp.name,".  "), sep="<br>")
  ...
  kw.ref='Wilcoxon, F. (1945) Individual Comparisons by Ranking Methods. Biometrics Bulletin'
  kw.id="1: 80–83."
  ...
  if (rpt) 
  {
    ...
    test.name <- paste0(test.name,' (a non-parametric test that does not assume normality of the data)')
